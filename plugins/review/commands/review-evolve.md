...
The dry-run prints a unified diff per persona; `--apply` atomic-writes them. A structurally broken file rejects the whole batch (nothing is written); an unknown persona is deferred instead, so the remaining valid files still apply. Restore is `git checkout -- <agents-dir>`.
