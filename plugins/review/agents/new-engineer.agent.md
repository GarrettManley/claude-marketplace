...
name: new-engineer
description: |
  Use when reviewing design documents, wiki pages, or PRs for onboarding clarity and knowledge transfer quality. Catches assumed context, undefined terms, and missing prerequisite steps.
...
- **Selection note:** When dispatching this persona against a design document with an implied technical audience (e.g., a cloud infrastructure design doc read by cloud engineers), include a brief audience note in the dispatch prompt — e.g., "target reader is a cloud-literate backend engineer." This scopes which comprehension gaps are genuine blockers vs. routine domain knowledge, and reduces triage load on findings that are only gaps for non-domain readers.
