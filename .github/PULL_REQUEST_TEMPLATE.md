...
- [ ] Updated `CHANGELOG.md` and bumped `plugins/<name>/.claude-plugin/plugin.json` version if a plugin changed (then ran `python3 ci/check-versions.py --fix` to sync `marketplace.json`)
- [ ] No machine-specific paths, corporate identifiers, or private project names in any changed file
